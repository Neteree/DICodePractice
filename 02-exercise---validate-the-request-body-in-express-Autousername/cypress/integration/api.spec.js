...
describe("API server", () => {
  ...
  it("POST /properties returns a 400 if an invalid request body is supplied", async () => {
    const body = {};

    await cy
      .request({
        url: "http://localhost:5001/properties",
        method: "POST",
        body,
        failOnStatusCode: false,
      })
  ...
  it("POST /properties returns a 400 if a negative askingPrice is supplied", async () => {
    const body = {
      description:
        "An easy living, conveniently located, brick & tile home on a highly desirable street and surrounded by quality homes.",
      address: "8 Shasta Pass",
