...
describe("App", () => {
  ...
  it("Should POST /chirps creates a chirp for the user with the unique name @Jeph, 201 Created", async () => {
    // Arrange
    const expectedStatus = 201;
    const chirpBody = {
      uniqueName: "@Jeph",
      text: ["What's happening?, how come I’m awake"],
    };

    // Act
    const response = await request(app).post("/chirps").send(chirpBody);

    // Assert
    expect(response.status).toBe(expectedStatus);
    expect(response.body).toEqual(expect.objectContaining(chirpBody));
    expect(response.body.uniqueName).toBe("@Jeph");
    expect(response.body.userName).toBeDefined();
    expect(response.body.text).toBeDefined();
    expect(response.body.avatar).toBeDefined();
    expect(response.body.replies).toBe(0);
    expect(response.body.rechirps).toBe(0);
